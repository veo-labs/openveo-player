...
(function(app) {
   ...
   *  - **oplPlay**: Player starts playing
   *  - **oplPause**: Player pauses
   *  - **oplLoadProgress**: Player is buffering
   *  - **oplPlayProgress**: Player is playing
   *  - **oplReady**: Player is ready to play the media
   *  - **oplDurationChange**: Media duration has changed
   *  - **oplEnd**: Media has reached the end
   *  - **oplWaiting**: Media is waiting for buffering
   *  - **oplPlaying**: Media is ready to play after buffering
   *  - **oplError**: Player as encountered an error
   *
   * e.g.
   *
   * // Get an instance of the OplVimeoPlayer
   * // (which extends OplPlayer)
   * var OplVimeoPlayer = $injector.get('OplVimeoPlayer');
   * var player = new OplVimeoPlayer(element, 'player_id', '118786909');
   *
   * @class OplPlayer
   */
  function OplPlayer() {
     ...
     *
     * @method init
     * @param {Object} jPlayerElement The JQLite HTML element corresponding to the HTML element which will receive
     * events dispatched by the player
  ...
  app.factory('OplPlayer', OplPlayer);
